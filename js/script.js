...
for (let i = 1; i <= frameCount; i++) {
    ...
    img.onload = handleImageProgress;
    img.onerror = () => {
        console.warn(`Missing or corrupt frame at: ${img.src}`);
        handleImageProgress(); 
    };
...
const mobileMilestones = [0, 30, 55, 80, 100];
...
menuLinks.forEach(link => {
    link.addEventListener("click", (e) => {
        ...
        if (isMobile) {
            ...
            const targetScroll = (targetMilestoneFrame / (frameCount - 1)) * maxScrollTop;
...
if (isMobile) {
    ...
    window.addEventListener("scroll", () => {
        ...
        targetFrame = Math.floor(scrollFraction * (frameCount - 1));
    });
}
